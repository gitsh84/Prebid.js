    ...
    try {
      ...
      if (bid) {
        ...
        if (renderer && renderer.url) {
          ...
          if (window.shinez && window.shinez.useConfiant) {
            try {
              // console.log("using confiant");
              ...
              var CONFIANT_CDN = 'clarium.global.ssl.fastly.net';
              var CONFIANT_ID = 'WAgMR5AArjr9TJ0QCrunNt93nUc';
              if (confiantExcludeBidders.includes(bid.bidder) || !confiantWrap(doc, ad, bid.bidder, width + 'x' + height, CONFIANT_CDN, CONFIANT_ID)) {
                doc.write(ad);
                doc.close();
              }
            } catch(e) {
